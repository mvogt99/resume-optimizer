...
>
> **Revision 2.1 (2026-09-05):** **Both parity risks CLOSED.** CloudLift reports and the committed evidence confirms **11 of 12 contracts in three-way parity, 0 divergences** (`docs/parity_three_way_results.md`).
>
> `IGraphDatabase` now HAS an AWS arm — the "Neptune is VPC-only at any setting" premise was **wrong and was never verified**; AWS added public Neptune endpoints in engine 1.4.6.x (IAM auth, public subnet, SG allowlist). Deployed and passing. **Drop the execution-location framing for Neptune** — it needs ordinary provisioning. That framing still holds for DocumentDB and ElastiCache, which genuinely are VPC-only.
>
> `IVectorSearch` now has an Azure arm: westus3 lacks `basic` AI Search capacity but **westus2 and southcentralus have it**. Cross-region costs nothing in fidelity — the suite compares values, never timings.
>
> `IDocumentDatabase`, `ICacheStore` and `ILifecycleDatabase` are three-way via an in-VPC CodeBuild runner. **`ILifecycleDatabase` was blocked by a CONTRACT DEFECT, not infrastructure**: it had no way to create a record, so no consumer could use it standalone in any environment without a document-database adapter on the same store just to seed. `create_memory()` now exists, idempotent, in all three adapters.
>
> ⚠ **`IFunctionExecution` is BLOCKED, not pending** — OpenWhisk's standalone images ship a Docker client (18.06.3-ce, API 1.38) this host's daemon rejects (requires 1.40+). AWS and Azure work; local does not, on this machine. **If IRI ever needs local function execution, design around it rather than waiting.**
>
