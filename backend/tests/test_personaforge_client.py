...
class TestPfRecall:
    ...
    def test_returns_context_string_on_success(self):
        ...
        with patch(
            "personaforge_client.requests.post", return_value=_mock_response(json_data=payload)
        ):
    ...
    def test_returns_none_on_connection_error(self):
        ...
        with patch(
            "personaforge_client.requests.post", side_effect=req.exceptions.ConnectionError()
        ):
    ...
    def test_returns_none_on_timeout(self):
        ...
        with patch("personaforge_client.requests.post", side_effect=req.exceptions.Timeout()):
            result = pfc.pf_recall("career_profile", "any task")
        assert result is None
    ...
    def test_returns_none_on_http_error(self):
        ...
        resp = _mock_response(
            status_code=500,
            raise_exc=req.exceptions.HTTPError(response=MagicMock(status_code=500, text="err")),
        )
        with patch("personaforge_client.requests.post", return_value=resp):
            result = pfc.pf_recall("career_profile", "any task")
        assert result is None
    ...
    def test_returns_none_on_missing_context_key(self):
        with patch("personaforge_client.requests.post", return_value=_mock_response(json_data={})):
            result = pfc.pf_recall("career_profile", "any task")
        assert result is None
    ...
    def test_caches_result(self):
        ...
        with patch("personaforge_client.requests.post", mock_post):
    ...
    def test_never_raises(self):
        with patch("personaforge_client.requests.post", side_effect=RuntimeError("boom")):
...
class TestPfRemember:
    def test_returns_none_on_success(self):
        with patch(
            "personaforge_client.requests.post", return_value=_mock_response(status_code=200)
        ):
...
class TestPfFeedback:
    def test_returns_none_on_success(self):
        with patch("personaforge_client.requests.post", return_value=_mock_response()):
