...
- **Configuration-driven, event-driven AWS data integration platform** serving 3 clients across 4 data domains and 20+ source systems as a canonical data product architecture — Apache Iceberg for storage, Collibra DQ for data quality, AWS Athena and Snowflake as the governed analytics consumption layer, and metadata-driven mappings governing ingestion through delivery at scale.
- **Manufacturing and consumer goods architecture** — designed a cloud data platform for a global manufacturing client while at AHEAD, scoped to architecture and design; client operates in label and packaging markets serving consumer-product categories.
- **Greenfield Databricks lakehouse** for a healthcare provider serving 10,000 customers — Unity Catalog governance and modeling standards established from inception through production.
...
SC Johnson's environment — global consumer products, manufacturing operations, supply chain, product and packaging data, finance, and commercial analytics — calls for enterprise architecture that governs data consistently across platforms rather than creating another generation of fragmentation. The experience here is directly relevant: Azure-centered cloud and data platform design, canonical data models for supply chain domains (orders, products, shipments, AP/GL) abstracted from SAP HANA and Oracle Financials, and a consistent record of building platforms that engineering organizations can run, govern, and extend independently. The value is not just designing the platform — it is ensuring the organization owns it long after the architecture is set.
